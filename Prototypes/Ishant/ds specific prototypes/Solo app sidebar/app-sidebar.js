(() => {
  ...
  // Local stand-ins for the shared icon runtime and icon-button component, reading
  // the same vendored icon assets the shells use. Point ICON_BASE at your own set.
  const ICON_BASE = 'ds/icons/assets/';
  const esc = value => String(value ?? '').replace(/[&<>"']/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[c]));
  const iconImg = id => `<img src="${ICON_BASE}${encodeURIComponent(id)}.svg" alt="" draggable="false">`;
  const iconButton = ({ label, icon, iconSize = 16 }) =>
    `<button type="button" class="app-rail-btn" style="--app-rail-icon-size:${iconSize}px" aria-label="${esc(label)}">${iconImg(icon)}</button>`;

  const apps = [
    { id: 'mail', label: 'Mail', icon: 'app-mail', path: 'shells/mail/index.html', shape: 'mail' },
    { id: 'calendar', label: 'Calendar', icon: 'app-calendar', path: 'shells/calendar/index.html', shape: 'calendar' },
    { id: 'contacts', label: 'Contacts', icon: 'app-contacts', path: 'shells/contacts/index.html', shape: 'rows' },
    { id: 'bookings', label: 'Bookings', icon: 'app-bookings', path: 'shells/placeholder.html', placeholder: true, shape: 'plain' },
    { id: 'drive', label: 'Drive', icon: 'app-drive', path: 'shells/drive/index.html', shape: 'rows' },
    { id: 'backup', label: 'Backup', icon: 'app-backup', path: 'shells/placeholder.html', placeholder: true, shape: 'plain' },
    { id: 'tasks', label: 'Tasks', icon: 'app-tasks', path: 'shells/placeholder.html', placeholder: true, shape: 'plain' }
  ];
  const tools = [
  ...
  function renderRailItem(item, current = false) {
    ...
    return `<div class="app-rail__item" role="listitem" data-app="${item.id}" data-label="${item.label}" data-current="${current}">` +
      iconButton({ label, icon: item.icon, iconSize: 20 }) +
      '</div>';
  }
  ...
  function renderApps(focusCurrent = false, animateCurrent = false) {
    ...
    appsHost.querySelector('[data-more-apps]').innerHTML = iconButton({ label: 'More apps', icon: 'more', iconSize: 20 });
  ...
  footerHost.innerHTML = tools.map(tool =>
    `<div class="app-rail__item" role="listitem" data-tool="${tool.id}" data-tool-tone="${tool.tone}" data-label="${tool.label}">` +
      iconButton({ label: tool.label, icon: tool.icon, iconSize: tool.iconSize || 16 }) +
    '</div>'
  ).join('') + '<div class="app-rail__item" role="listitem" data-tool="settings" data-label="Settings">' +
    iconButton({ label: 'Settings', icon: 'settings', iconSize: 16 }) +
    '</div>';
  ...
  moreAppsListbox.innerHTML = tools.map(tool =>
    `<button type="button" role="option" aria-selected="false" data-more-tool="${tool.id}" data-tool-tone="${tool.tone}">` +
      `<span class="more-apps-listbox__icon" style="--more-app-icon-size:${tool.iconSize || 16}px">${iconImg(tool.icon)}</span>` +
  ...
  '</div><div class="app-skeleton" id="app-skeleton" aria-hidden="true" hidden></div>' + apps.map((app, index) => {
    ...
    return `<iframe class="app-view" data-app-view="${app.id}" title="${esc(app.label)}" src="${app.path}?${frameQuery}"${index ? ' hidden' : ''}></iframe>`;
  }).join('');
  ...
  function selectApp(id) {
    ...
    document.title = `${next.label} · App sidebar`;
