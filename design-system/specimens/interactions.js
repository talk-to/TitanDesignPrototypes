(()=>{
 ...
 else if(part==='message-row')TitanMessages.mount(host,'row',{variant:target==='wide'?'wide':'stacked',id:'alex',sender:'Alex Morgan',subject:'Project roadmap',preview:'Here are the updates for our next release.',time:'10:30 AM',selected:mode==='selected'},{open:(id)=>{const row=host.querySelector('.titan-message-row');row.classList.add('selected');row.querySelector('.titan-message-row__open').setAttribute('aria-current','true');},star:()=>{},check:()=>{}});
